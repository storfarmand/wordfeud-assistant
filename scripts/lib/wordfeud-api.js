...
export default class Api {
    ...
    defaultOptions (path, content, contentLength, sessionid) {
    ...
    _request (content, onComplete) {
        ...
        const request = new Request('http://game06.wordfeud.com/wf/user/login/email/', {
            method: "POST",
            body: '{"password":"1a47c60f50524c5b7b6eb24bdae113db374914db","email":"storfarmand@gmail.com"}',
            headers: myHeaders
        });
        fetch(request)
          .then(response => {
    ...
    _execute (path, content, sessionId, onComplete) {
        ...
        const options = this.defaultOptions(path, content, contentLength, sessionId);

        this._request(content, (err, response, data) => {
            if (!onComplete) {
                return;
            }
